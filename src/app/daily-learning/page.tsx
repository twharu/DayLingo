...
export default function Home() {
  ...
  const handleDateChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (userId) {
      loadTasksForDate(userId, newDate);
    }
  };
                ...
                <div className="space-y-2">
                  {tasks.map((task) => (
                    <div
                      ...
                      onClick={() => {
                        router.push(`/vocabulary?date=${task.date}&taskName=${encodeURIComponent(task.taskName)}`);
                      }}
                    >
                      <div className="flex items-center justify-between">
...
詳細描述: ${currentTaskName}
                  ...
                  <button
                    ...
                    className="w-full bg-blue-400 text-white px-6 py-3 rounded-lg hover:bg-blue-700 transition-colors font-medium flex items-center justify-center space-x-2"
                  >
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 4v16m8-8H4"/>
                    </svg>
              ...
              <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50" onClick={() => setSelectedWordIndex(null)}>
                <div className="bg-white rounded-lg shadow-xl p-8 max-w-2xl mx-4 max-h-[80vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
                  ...
                  <div className="mt-6 flex gap-3">
                    ...
                    >
                      {savingWords.has(selectedWordIndex) ? '儲存中...' : savedWords.has(selectedWordIndex) ? '已收藏' : '收藏'}
                    </button>
                    <button
                      onClick={() => setSelectedWordIndex(null)}
